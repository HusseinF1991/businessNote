...
public class MySqliteDB extends SQLiteOpenHelper {
    ...
    public MySqliteDB(Context context ,String sqliteDbName) {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        if(sqliteDbName.equals("BusinessNoteDb_Main")){

            sqLiteDatabase.execSQL("DROP TABLE  IF EXISTS agentsDb");
            sqLiteDatabase.execSQL("CREATE TABLE IF NOT EXISTS `agentsDb` (" +
                    "Id INTEGER NOT NULL primary key AUTOINCREMENT, " +
                    "dbName varchar(100) NOT NULL, " +
                    "dbSecretKey DateTime NOT NULL) ");
        }
        ...
        contentValues.put("fromWhere" , "admin");
    ...
    public Cursor GetAllDb_agentsDb(){
        Log.d("sqlite_db" , "GetAllDb_agentsDb_Started");

        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        Cursor cursor = sqLiteDatabase.rawQuery("Select * From agentsDb"  , null);

        //sqLiteDatabase.close();
        Log.d("sqlite_db", "row:"+cursor.getCount());
        Log.d("sqlite_db" , "GetAllDb_agentsDb_Completed");

        return cursor;
    }


    public Long InsertNewDb_agentsDb(String dbName , String dbSeretKey){

        Log.d("sqlite_db" , "InsertNewDb_agentsDb_started");

        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put("dbName" , dbName);
        contentValues.put("dbSecretKey" ,dbSeretKey );

        Long result = sqLiteDatabase.insert( "agentsDb" , null , contentValues);

        Log.d("sqlite_db" , "InsertNewDb_agentsDb" + result);

        return result;
    }


    public boolean DeleteDb_agentsDb(String dbName){

        Log.d("sqlite_db" , "DeleteDb_agentsDb_started");

        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();

        int result = sqLiteDatabase.delete("agentsDb" , "dbName = '"+ dbName +"'" , null);
    ...
    public boolean UpdateDbSecretKey_agentsDb(String dbName, String dbSecretKey){

        Log.d("sqlite_db" , "UpdateDbSecretKey_agentsDb_started");

        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        int result = -1;
        ContentValues contentValues = new ContentValues();
        contentValues.put("dbSecretKey" , dbSecretKey);
        result = sqLiteDatabase.update("agentsDb" ,contentValues, "dbName = '"+ dbName +"'" , null);
